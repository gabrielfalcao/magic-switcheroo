...
pub fn write_file(filename: String, data: Vec<u8>) -> Result<(), Box<dyn Error>> {
...
mod tests {
    ...
    fn create_test_image_file(filename: String) -> Result<String, Box<dyn Error>> {
        write_file(filename.clone(), test_image_data())?;
        Ok(filename)
    }
    #[test]
    fn test_enchant_file() -> Result<(), Box<dyn Error>> {
    ...
    #[test]
    fn test_restore_file() -> Result<(), Box<dyn Error>> {
